...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"ramdisk_load_address=0x800000\0"	\
	"devicetree_image=devicetree.dtb\0"	\
	"devicetree_load_address=0x1000000\0"	\
	...
	"mmc_loadbit=echo Loading bitstream from SD/MMC/eMMC to RAM.. && " \
		"mmcinfo && " \
		"load mmc ${bootpart} ${loadbit_addr} ${bootdir}/${bitstream_image} && " \
		"fpga loadb 0 ${loadbit_addr} ${filesize}\0" \
	"sdboot=if mmcinfo; then " \
			"run uenvboot; " \
			"run mmc_loadbit; " \
			"echo Copying Linux from SD to RAM... && " \
			"load mmc ${bootpart} 0x8000 ${bootdir}/${kernel_image} && " \
			"load mmc ${bootpart} 0x1000000 ${bootdir}/${devicetree_image} && " \
			"bootz 0x8000 - 0x1000000;" \
		"fi\0" \
